...
const Foam::debug::tolerancesSwitch Foam::isoCutFace::pointMatchTol_
(
    "isoCutFacePointMatchTol",
    ROOTVSMALL
);


const Foam::debug::tolerancesSwitch Foam::isoCutFace::isoValueTol_
(
    "isoCutFaceIsoValueTol",
    1e-10
);


const Foam::debug::tolerancesSwitch Foam::isoCutFace::deltaTFractionTol_
(
    "isoCutFaceDeltaTFractionTol",
    1e-6
);


const Foam::debug::tolerancesSwitch Foam::isoCutFace::deltaTTol_
(
    "isoCutFaceDeltaTTol",
    1e-10
);
...
const Foam::debug::tolerancesSwitch Foam::isoCutFace::maxTSmallTol_
(
    "isoCutFaceMaxTSMallTol",
    1e-20
);
...
)
{
    ...
    if (mag(f1 - isoValue) < isoValueTol_())
    {
        f1 += sign(f1 - isoValue)*isoValueTol_();
    }
    ...
    forAll(pLabels, pI)
    {
        ...
        scalar f2 = f[pl2];
        if (mag(f2 - isoValue) < isoValueTol_())
        {
            f2 += sign(f2 - isoValue)*isoValueTol_();
        }
...
)
{
    ...
    DynamicList<scalar> sortedTimes(pTimes.size());
    {
        ...
        const scalar tSmall = max(deltaTFractionTol_()*dt, maxTSmallTol_());
        forAll(order, ti)
        {
...
) const
{
    ...
    if (np0 && np1)
    {
        ...
        if (Bx > pointMatchTol_())
        {
            // If |AB| > 0 ABCD we use AB to define xhat
            xhat = (B - A)/mag(B - A);
        }
        else if (mag(C - D) > pointMatchTol_())
        {
